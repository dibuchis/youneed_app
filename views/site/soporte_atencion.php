...
<div class="container-crud atenciones-form" style="margin-top: 20px;">
	...
	<div class="row">
		<div class="col-md-5">
			...
			<div class="row">
				<div class="col-md-6">
					<?= $form->field($model, 'alianza_id')->widget(\kartik\widgets\Select2::classname(), [
		                'data' => \yii\helpers\ArrayHelper::map(\app\models\Alianzas::find()->orderBy('nombre')->asArray()->all(), 'id', 'nombre'),
		                'options' => ['placeholder' => Yii::t('app', 'Seleccione alianza')],
		                'pluginOptions' => [
		                    'allowClear' => true
		                ],
		            ]); ?>
				    ...
				    <?= $form->field($model, 'numero_celular')->textInput() ?>
				</div>
				<div class="col-md-6">
					<?php 
		                echo $form->field($model, 'fecha_nacimiento')->widget(\janisto\timepicker\TimePicker::className(), [
		                    //'language' => 'fi',
		                    'mode' => 'date',
		                    'clientOptions'=>[
		                        'dateFormat' => 'yy-mm-dd',
				    ...
				    <?= $form->field($model, 'ciudad_id')->widget(\kartik\widgets\Select2::classname(), [
		                'data' => \yii\helpers\ArrayHelper::map(\app\models\Ciudades::find()->orderBy('nombre')->asArray()->all(), 'id', 'nombre'),
		                'options' => ['placeholder' => Yii::t('app', 'Seleccione ciudad')],
		                'pluginOptions' => [
		                    'allowClear' => true
		                ],
		            ]); ?>
